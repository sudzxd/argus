...
Four strategies run independently and their results are merged by the Context Ranker. The total retrieval budget (60% of token budget) is split: structural 40%, lexical 30%, semantic 20%, agentic 10%.

- **Structural** (40%) — Walks the dependency graph from changed files. Dependents (what calls the changed code) are scored at 0.9 relevance; dependencies (what the changed code calls) at 0.8. Deterministic and instant.
...
- **Agentic** (10%, optional) — LLM generates a `SearchPlan` (structured output with `queries` + `needs_more_context`), iterates up to 3 times to find context that other strategies miss. Requires `enable_agentic: true`.
