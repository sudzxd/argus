...
const features = [
  {
    ...
    accent: "from-violet to-violet-light",
  },
  {
    icon: (
      <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  },
  {
    ...
    description:
      "Supports Anthropic Claude, OpenAI GPT, and Google Gemini. Switch models with a single config line — no code changes needed.",
    accent: "from-amber to-yellow-300",
  },
  {
    icon: (
      <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  },
  {
    ...
    accent: "from-violet-light to-cyan",
  },
  {
    icon: (
      <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    description:
      "Learns your codebase's conventions and anti-patterns over time. Reviews get smarter with every indexed commit.",
    accent: "from-pink-400 to-violet",
  },
];
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
          {features.map((feature, i) => (
            <GlassCard key={feature.title} delay={i * 0.1}>
              <div
                className={`w-10 h-10 rounded-lg bg-gradient-to-br ${feature.accent} flex items-center justify-center mb-4 opacity-80`}
              >
                {feature.icon}
              </div>
              <h3
